CLASS zcl_data_file_upload_a02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS: get_input_field_value IMPORTING name
                                   TYPE string dataref TYPE data
...
CLASS zcl_data_file_upload_a02 IMPLEMENTATION.
  ...
  METHOD if_http_service_extension~handle_request.
    ...
    CASE request->get_method( ).
      WHEN CONV string( if_web_http_client=>get ).
        ...
        ENDIF.
*       ELSE.
*           DATA(name_filter) = xco_cp_abap_repository=>object_name->get_filter(xco_cp_abap_sql=>constraint->contains_pattern(to_upper( sap_table_request ) && '%' ) ).
            ...
            IF sy-subrc = 0.
                ...
                READ TABLE content_dis REFERENCE INTO DATA(content_dis_item) INDEX 3.
                    IF sy-subrc = 0.
                        ...
                        REPLACE ALL OCCURRENCES OF `"` IN filename
                            WITH space.
...
*       Check table name is valid.
        IF
*           xco_cp_abap_repository=>object->table->database_table->for(
*           iv_name = CONV #( tablename ) )->exists( ) = abap_false
*           OR
            tablename IS INITIAL.
            response->set_status( i_code = if_web_http_status=>bad_request i_reason = |Table name { tablename } not valid or does not exist| ).
...
*       Check file extension is valid, only json today.
        IF fileext <> `json`.
            response->set_status( i_code =
            if_web_http_status=>bad_request i_reason = `File type not supported` ).
...
* Load the data to the table via dynamic internal table
        ...
        /ui2/cl_json=>deserialize( EXPORTING json = filedata pretty_name = /ui2/cl_json=>pretty_mode-none CHANGING data = <table_structure> ).
  ...
  METHOD get_input_field_value.
    ...
    FIELD-SYMBOLS: <value> TYPE data, <field> TYPE any.
